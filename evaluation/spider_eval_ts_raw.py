...
"""
From: https://github.com/taoyds/test-suite-sql-eval/blob/master/evaluation.py
"""
...
from evaluation.test_suite_sql_eval.exec_eval import eval_exec_match
from evaluation.test_suite_sql_eval.process_sql import Schema, get_schema, get_sql
...
def evaluate(
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        if (i + 1) % 10 == 0:
            print(
                "Evaluating %dth prediction (%.2f%%)"
                % (i + 1, (i + 1) / len(plist) * 100)
            )
        scores["joint_all"]["count"] += 1
        turn_scores = {"exec": [], "exact": []}
        for idx, pg in enumerate(
            tqdm(zip(p, g), desc="Evaluating", total=len(p), colour="green", ncols=100)
        ):
            p, g = pg
            p_str = p[0]
    ...
    _keys = ["easy", "medium", "hard", "extra", "all"]
    _values_ex = "\t".join([f"{execution_scores[key]*100:.4f}" for key in _keys])
    _values_em = "\t".join([f"{exact_match_scores[key]*100:.4f}" for key in _keys])
    print(f"Execution Scores: {_keys}: {_values_ex}")
    print(f"Exact Match Scores: {_keys}: {_values_em}")
    return execution_scores, exact_match_scores
...
if __name__ == "__main__":
    ...
    ), "test suite database not found. download and rename it to `evaluation/test_suite_sql_eval/testsuite_database`"
