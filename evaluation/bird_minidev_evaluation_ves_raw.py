...
from evaluation.bird_minidev_evaluation_utils_raw import (
    connect_db,
    execute_sql,
    load_json,
    package_sqls,
...
def execute_sql(sql, db_path, sql_dialect, return_time=False):
    ...
    cursor.execute(sql)
    res = cursor.fetchall()
    res = round_floats_in_structure(res)
...
if __name__ == "__main__":
    ...
    python -u evaluation/bird_minidev_evaluation_ves_raw.py \
        ...
        --predicted_sql_path results_baseline/BIRD-minidev-baseline/predict_mini_dev_gpt-4-turbo_sqlite.json \
