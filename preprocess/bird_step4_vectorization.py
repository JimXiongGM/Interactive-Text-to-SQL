...
def make_col_embedding_text(d):
    """
    bird has name table column_description
    """
    col_name = d["name"].replace("_", " ").lower()

    # if d["column_description"] and d["name"].lower() != d["column_description"].lower():
    #     x=1
    desc = d["column_description"] if d["column_description"] else "no description."
    if desc[-1] not in [".", "?", "!"]:
        desc += "."
    ...
    embedding_text = f"a column named {col_name} in table {t_name} about {desc}".lower()
...
def vectorization_cols():
    ...
    # chroma
    chroma_client = init_chroma_client(name="bird")
    index_name = os.path.basename(COL_VEC_PKL_FILE).replace(".pkl", "")
    names = [i.name for i in chroma_client.list_collections()]
...
if __name__ == "__main__":
    # python preprocess/bird_db_vectorization.py
